...
const SearchInput = () => {
  ...
  const delayCallback = useDebounceCallback(800)
  ...
  const handleSearchOptionChange = (selectedOption: SelectOptionType) => {
    ...
    const productName = (selectedOption as ISelectOption).value as string

    if (productName) {
      getPartAndRedirect(productName)
    }
  ...
  const searchPart = async (search: string) => {
    try {
      ...
      const names = data
        .map((item: IBoilerPart) => item.product_name)
  ...
  const onSearchInputChange = (searchText: string) => {
    ...
    delayCallback(() => searchPart(searchText))
  }
  ...
  const onSearchMenuOpen = () => {
    ...
    setOnMenuOpenContainerStyles({
      boxShadow: '0p 2px 8px rgba(0, 0, 0, 0.1)',
    })
